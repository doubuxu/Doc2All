...
* **关键属性**：
  ...
  * `src`：必须指向子图文件名，后缀为 png（如 `./table_1.png` 或 `./fig_1.png`）。
...
* 整个 PPT 页面必须被 `<div class="ppt-slide">` 包裹。
* `#ppt-canvas` 必须作为 `.ppt-slide` 的直接子元素。
...
* **全局容器**：根容器为 `div#ppt-canvas`，宽高需与原图一致。
* **多栏布局**：使用 `main` 标签作为主体，内部划分若干 `.column`。
* **位置对齐**：严格参考 XML 的 bbox 坐标，通过 flex / grid 确保元素顺序与物理位置对齐。
...
<header>
    ...
    <div class="header-logos">
        ...
        <img src="./fig_1.png" style="width: 100px; height: 100px;" data-semantic="校徽">
    </div>
</header>
...
<main>
    <div class="column">
        <section class="layout-block" data-section-id="section_1">
            ...
            <div class="figure-container">
                <img src="./fig_1.png" style="width: 500px; height: 300px;" data-semantic="模型架构图">
            </div>
...
if __name__ == "__main__":
    ...
    ppt_path='../pptDataOutput/VLONTCMVI6XS2XAUCTY65H2M2LJXSUIQ'
    index=0
    html_code=singele_poster_process(ppt_path,index)
    print(html_code)
    html_path=f'./VLONTCMVI6XS2XAUCTY65H2M2LJXSUIQ_0.html'
