...
"""
    prompt="""
### 任务目标
你是一个顶尖的前端布局专家。你的任务是编写一段单文件的 HTML 代码，在实现完美响应式适配的同时，务必在宽屏模式下（宽度接近 max-width 时）1:1 精准复刻海报的原始物理布局。该 HTML 必须严格遵循以下结构协议，以便后续自动化脚本解析。
...
- **图片标签 `<img>` 规范**：
  ...
  - **语义描述**：必须包含 **`data-semantic`** 属性，根据视觉内容准确描述该图（如 `data-semantic="消融实验结果柱状图"`）。
...
- **内容完整性**：必须逐条核对 `content_list.json`，确保所有 text 文本 100% 还原，严禁遗漏。根据图片视觉特征估算并设定合理的 `font-size` 和 `gap`。
...
4. 所有布局参数（gap, width, padding）必须参考视觉特征和 bbox 坐标。"""},
        ...
        {"role": "user", "content":[
            ...
            *multimodal_content,
            {
                "type":"text",
                "text":"我将给出mineru版面识别的content_list.json结果，内容是mineru识别的元素阅读顺序和个元素的bbox等信息，请你参考该json(特别是bbox这一位置信息)来辅助生成最终的html代码。content_list:"
            },
            {
                "type":"text",
                "text":content_list
...
if __name__ == "__main__":
    ...
    html_code=singele_poster_process(img_path,poster_name,sub_image_path)
    html_path=f"./{poster_name}6.html"
    save_html(html_path,html_code)