...
<table><tr><td>X</td><td>A</td><td>D</td><td>A</td><td>1</td><td>n</td></tr><tr><td>Node Feat.</td><td>Adjacency Mx.</td><td>Degree Mx.</td><td>D-1/2AD-1/2</td><td>All-one Vec.</td><td>Node Num.</td></tr></table>
...
$$
\mathbf {r} _ {A} = \tilde {\mathbf {A}} ^ {t} \mathrm {M L P} (\mathbf {X}), \quad \mathbf {r} = \operatorname {s g n} \left(\mathbf {r} _ {A} \mathbf {1} ^ {\top} - \mathbf {1 r} _ {A} ^ {\top}\right) \mathbf {1}. \tag {1}
$$

Weusethe sigmoid function to approximate the sign function in backpropagation.
...
$$
\hat {\mathbf {P}} = \exp \left\{- \tau \left[ \left(\mathbf {m} \mathbf {1} ^ {\top} - \mathbf {1 r} ^ {\top} + n\right) \quad (\mathrm {m o d} n) \right] \right\}, \tag {2}
$$
...
The Compressed Convolution Network (CoCN) can be highlighted as:
